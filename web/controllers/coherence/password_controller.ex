defmodule ActiveMonitoring.Coherence.PasswordController do
  @moduledoc """
  ...
  * update - verify password, password confirmation, and update the database
  ...
  alias ActiveMonitoring.Coherence.Helper
    ...
    case user do
      ...
      user ->
        if Helper.expired? user.reset_password_sent_at, days: Config.reset_token_expire_days do
          ...
          conn
          |> put_layout({ActiveMonitoring.LayoutView, "app.html"})
          |> put_view(Coherence.PasswordView)
          |> render("edit.html", changeset: ActiveMonitoring.User.changeset(user))
        end
    end
  end
